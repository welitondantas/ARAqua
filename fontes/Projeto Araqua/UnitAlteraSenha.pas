      ...
      begin
       if (Length(EditNova.Text) >= 3) then
          begin
              ...
              begin
                ...
                FDQueryAlterar.ParamByName('IDUSUARIO').AsInteger := FormPrincipal.idUsuario;
                  try
                    ...
                    ShowMessage('Não foi possível alterar a senha do usuário.'+#13+'Motivo do erro: '+e.Message);
                  end;
                Close;
              end
              else
              begin
                ShowMessage('As senhas informadas não coincidem. Verifique se as senhas são iguais e tente novamente.');
                EditNova.Text := '';
                EditConfirma.Text := '';
                EditNova.SetFocus;
              end;
            ...
            ShowMessage('A senha informada não atende aos requisitos mínimos de segurança. Verifique!');
      end
      else
      begin
        ShowMessage('A senha atual não corresponde com a senha do usuário. Verifique!');
        EditSenhaAtual.Text := '';
